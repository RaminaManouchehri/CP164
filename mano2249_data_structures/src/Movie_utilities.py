    ...
    while value != "" or genres == []:
        if value.isdigit():
            ...
            if number >= n:
                ...
                print(f"Error: not a positive number")
...
def get_by_genres(movies, genres):
    ...
    gmovies = []
    for i in movies:
        if len(i.genres) == len(genres):
            match = True
            j = 0
            while match and j < len(genres):
                if genres[j] not in i.genres:
                    match = False
                else:
                    j += 1
...
def genre_counts(movies):
    ...
    for i in range(len(Movie.GENRES)):
        gmovies = get_by_genre(movies,i)
        counts.append(len(gmovies))
    return counts
